...
import { MessageCircle, Instagram, Facebook, Send, Music4, Globe2, Plus, X } from "lucide-react";
      ...
      <section className="mx-auto max-w-7xl px-4 py-14">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className={`relative inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r ${gradient} text-black font-semibold`}
          >
          ...
          <motion.div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }} className="w-full max-w-xl rounded-2xl border border-white/10 bg-[#161626] p-6">
              ...
              <div className={`mt-4 flex ${dir==='rtl' ? 'flex-row-reverse' : ''} gap-2`}>
                <button onClick={addVideo} className={`relative inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r ${gradient} text-black font-semibold`}>
              ...
              <p className="mt-3 text-xs text-white/50">Tip: paste any Google Drive link; we auto-convert it to an embeddable preview.</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
